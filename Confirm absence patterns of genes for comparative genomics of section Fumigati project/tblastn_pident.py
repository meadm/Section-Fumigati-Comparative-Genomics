...
#add n argument that is the percent identity cutoff. represented by whole numbers, and not a decimal
parser.add_argument("-i","--pident",type=int,help="percent identity cutoff, ex. 30 for 30% or higher")
...
for line in l:
    ...
    cmd = "tblastn -db ../../Jacobs_genomes/all_in_one_stripped.fna -query " + query + " -outfmt '10 qaccver saccver pident length qlen evalue sstart send' -evalue 1e-10 -out " + out + " -taxids " + taxa
    #run the tblastn command
    os.system(cmd)
    #variable that will be the name of the filtered output file
    filtered = out + '.' + str(args.pident) + '_filtered'
    #pandas dataframe of the tblastn output that has all the lines and has headers
    df = pd.read_csv(out, header=None, names = ['qaccver', 'saccver', 'pident', 'length', 'qlen', 'evalue', 'sstart', 'send'])
    ...
    df[(df.pident >= args.pident) & (df.Coverage >= 0.7)].to_csv(filtered, header=None, index=None, sep='\t')
